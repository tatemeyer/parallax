...
///
/// `literal_separator` is enabled so a single `*` does not cross a
/// directory boundary — `globset`'s default lets `*` match `/`, which
/// would make `runs/*/verdict.md` indistinguishable from `runs/**/
/// verdict.md`. `**` is unaffected by this setting; it always crosses
/// directories.
...
mod scan_tests {
    ...
    fn figure_artifacts_report_their_size_and_never_their_pixels() {
        // `\x89` is not a valid escape in a `&str` literal (hex escapes
        // are ASCII-only), so the raw PNG-magic bytes are written
        // directly rather than through the `&str`-based `tree` helper.
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join("out/field.png");
        std::fs::create_dir_all(path.parent().unwrap()).unwrap();
